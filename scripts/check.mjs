...
const GATES = [
  ...
  },
  {
    ...
    available: () =>
      needsDevDep('dependency-cruiser') ?? needsFile('.dependency-cruiser.cjs') ?? needsFile('src'),
  },
  {
    name: 'test',
...
if (blocking.length === 0) {
  console.log(`Tudo verde (${results.length - skipped.length} executados, ${skipped.length} pulados).`);
  process.exit(0);
}
