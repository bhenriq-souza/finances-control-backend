...
function parseFrontmatter(raw, file) {
    ...
    for (const line of match[1].split(/\r?\n/)) {
        ...
        // Aspas simples ou duplas: formatadores de Markdown reescrevem o frontmatter.
        if (entry) frontmatter[entry[1]] = entry[2].trim().replace(/^['"]|['"]$/g, '');
    }
    return frontmatter;
}
...
for (const [id, { file, dependsOn }] of specs) {
    for (const dependency of dependsOn) {
        if (!specs.has(dependency))
            fail(file, `depends_on referencia spec inexistente "${dependency}"`);
    }
    const seen = new Set();
...
if (existsSync(BACKLOG)) {
    ...
    backlog.forEach((line, index) => {
        ...
        if (!specs.has(task[1])) {
            fail(
                location,
                `tarefa T-${task[1]}-${task[2]} referencia spec inexistente "${task[1]}"`,
            );
        }
        ...
        if (!/\b(AC|INV)-\d{4}-\d{2}\b/.test(block)) {
            fail(
                location,
                `tarefa T-${task[1]}-${task[2]} não referencia nenhum AC-* ou INV-* (INV-0000-02)`,
            );
        }
    });
}
